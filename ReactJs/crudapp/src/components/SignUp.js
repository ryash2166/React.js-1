...
function SignUp() {
  ...
  useEffect(()=>{
    setData(JSON.parse(localStorage.getItem('crudUser')))
  },[])

  const SignUpSchema = Yup.object({
    username:Yup.string().min(2).max(18).required(" * This Fild is Required"),
    email:Yup.string().email().required(" * This Fild is Required"),
    password:Yup.string().min(5).max(10).required(" * This Fild is Required"),
    confirm_password:Yup.string().oneOf([Yup.ref('password'),null],'Password Must Match').required('* This Fild is Required'),
  })

  function setLocalData (values) {
    if(!localStorage.getItem('crudUser')){
      localStorage.setItem('crudUser',JSON.stringify([values]))
      Success_Alert('User Data Add SuccessFully')
      handleReset();
    }else{
      const findUser = data.find(X =>(X.email === values.email || X.username === values.username))
      if(findUser === undefined){
        localStorage.setItem('crudUser',JSON.stringify([...data,values]))
        Success_Alert('User Data Add SuccessFully')
        setTimeout(() => {
          handleReset();
          neviGatLogin('/login')
        ...
        Error_Alert('User is already existing try')
        setTimeout(() => {
          Error_Alert('Try new Email Or UserName')
          handleReset();
        }, 1500);
      }
    }
          ...
          <div className="max-w-md w-full p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                />
              <p className="pt-1 text-red-500">{errors.username && touched.username ? errors.username : null}</p>
              </div>
              <div>
                <label
                  htmlFor="email"
              ...
              <div>
                ...
                <input
                  ...
                  className="mt-1 p-2 w-full border rounded-md focus:border-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300 transition-colors duration-300"
                />
                <p className="pt-1 text-red-500">{errors.confirm_password && touched.confirm_password ? errors.confirm_password : null}</p>
              </div>
              <div>
                <button
                  type="submit"
